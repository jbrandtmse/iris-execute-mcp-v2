 ...
 * constructs a real `@iris-mcp/admin` server or drives `prompts/list` — they
 * import prompt content directly from built dist output. This test proves
 * the Story 25.1 prompt content is actually WIRED to the real server
 ...
 * THIS server specifically — `promote-environment-change` is the gated
 * prompt that would (in a future epic) live on iris-admin-mcp.
 */
...
/** The prompts this package (iris-admin-mcp) owns per Story 25.1 AC 25.1.1. */
const OWN_PROMPT_NAMES = ["provision-project-environment", "audit-security-posture"];
...
describe("iris-admin-mcp real server prompts/list (AC 25.1.1, AC 25.0.3)", () => {
  ...
  it("the gated prompt 'promote-environment-change' is NOT registered on this server (AC 25.1.5)", async () => {
    const server = makeRealServer();
    const result = await callRequest(server, "prompts/list", {});
    const names = (result.prompts as Array<{ name: string }>).map((p) => p.name);
