/**
 * Story 30.1 — Preset roster coverage + count guards for iris-admin-mcp.
 *
...
describe("iris-admin-mcp tool visibility presets (Story 30.1)", () => {
  ...
  it("pins the exact roster sizes from spec §2.5 (admin: core 12/14, developer 10/16)", () => {
    expect(toolPresets.core.include.length).toBe(12);
    expect(toolPresets.core.exclude.length).toBe(14);
    expect(toolPresets.developer.include.length).toBe(10);
    expect(toolPresets.developer.exclude.length).toBe(16);
  });
