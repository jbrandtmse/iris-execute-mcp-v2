/**
 * Story 30.1 — QA end-to-end coverage for iris-admin-mcp's wired `toolPresets`.
 *
 ...
 * `tools/list`/`tools/call` and the `TOOL_PAIRS` proof; admin-mcp owns no
...
describe("E2E — iris-admin-mcp real McpServerBase construction with the wired toolPresets roster", () => {
  it("IRIS_TOOLS_PRESET=core yields exactly core.include + iris_server_profiles", () => {
    ...
    // Representative admin-only tools stay hidden under core.
    expect(server.getToolNames()).not.toContain("iris_audit_manage");
    expect(server.getToolNames()).not.toContain("iris_ldap_manage");
  });
  ...
  it("IRIS_TOOLS_PRESET=developer yields exactly developer.include + iris_server_profiles", () => {
    ...
    // Security/enterprise admin stays hidden even under developer.
    expect(server.getToolNames()).not.toContain("iris_user_manage");
    expect(server.getToolNames()).not.toContain("iris_role_manage");
  });
