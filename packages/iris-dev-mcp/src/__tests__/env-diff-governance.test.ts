/**
 * Story 27.0 AC 27.0.3 — governance-defaults proof for `iris_env_diff`.
 *
 * `iris_env_diff` adds ONE new post-foundation governance key to the
 * iris-dev-mcp server (`iris_env_diff`, scalar `mutates: "read"` — no action
 ...
 *
 * Mirrors `loc-governance.test.ts`. Runs in the DEFAULT vitest suite — no
 * live IRIS; the default profile's startup HEAD/GET are stubbed via a fetch
 * mock, and the tool handler is replaced with a spy so invocation is
 * observable.
 */
 ...
 * governance key from the tool name + `mutates`, so keeping the REAL schema
 * and `mutates` is what makes this an end-to-end proof.
 */
function spiedTool(spy: ReturnType<typeof vi.fn>): ToolDefinition {
  return { ...envDiffTool, handler: spy };
}
...
// ════════════════════════════════════════════════════════════════════
// AC 27.0.3 — the read tool is default-ENABLED under empty config.
// ════════════════════════════════════════════════════════════════════

describe("iris_env_diff governance default (AC 27.0.3)", () => {
  const env = makeEnvHarness();
  beforeEach(env.setup);
  afterEach(env.teardown);

  it("under EMPTY IRIS_GOVERNANCE, iris_env_diff is ALLOWED (read default-enabled; handler runs)", async () => {
    stageDefaultStartup(env.fetchMock);

    const handlerSpy = vi.fn(async () => ({
      content: [{ type: "text" as const, text: "ran" }],
      structuredContent: { summary: { driftCount: 0, identicalCount: 0 } },
    }));
  ...
  it("registers without throwing (the NEW key carries a scalar `mutates` class)", async () => {
    ...
    const server = new McpServerBase(makeServerOpts([envDiffTool]));
    await expect(server.start("stdio")).resolves.not.toThrow();
  });

  it("an explicit IRIS_GOVERNANCE override can still disable iris_env_diff", async () => {
    stageDefaultStartup(env.fetchMock);
    process.env.IRIS_GOVERNANCE = JSON.stringify({
      global: { iris_env_diff: false },
    });
