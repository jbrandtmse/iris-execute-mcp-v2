...
describe("iris-dev-mcp", () => {
  ...
  describe("tools/index.ts", () => {
    ...
    it("should export the document CRUD, compilation, intelligence, format, SQL, server, global, execute, LOC, env-diff, env-promote, and test-status tools", () => {
      ...
      expect(names).toContain("iris_test_status");
    });
  ...
  describe("McpServerBase instantiation with doc tools", () => {
    it("should create a server instance with 28 package tools (+1 framework tool)", () => {
      ...
      // 29 package tools + the framework `iris_server_profiles` discovery tool
      // (Epic 19, decision E1 — registered on every server by the shared base).
      expect(server.toolCount).toBe(30);
    });
