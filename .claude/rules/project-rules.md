...
## 45. Cross-package validation tests live in the all-packages aggregator, never in `@iris-mcp/shared`

**Context:** A default-suite test (or CI script) that needs metadata from EVERY server package's tools at once — tool `annotations` (e.g. the Rule #44 `readOnlyHint` cross-check), tool `name`s (e.g. `validate-prompts` proving every `iris_*` token in prompt/skill bodies is real), or any future cross-server catalog/lockstep check.

**Rule:** Such a test CANNOT live in `packages/shared/src/__tests__/`. `@iris-mcp/shared` is a DEPENDENCY of the five leaf server packages (`iris-dev/admin/interop/ops/data-mcp` each declare `@iris-mcp/shared: workspace:*`), and shared must NOT depend on them — importing a leaf package into shared is a circular dependency. The only package that legitimately depends on all five is the aggregator **`packages/iris-mcp-all`** (`@iris-mcp/all`). Put cross-package validation there. If `iris-mcp-all` is still a dependency-only stub, give it a minimal `vitest.config.ts` (mirror a sibling) + a `test` script + `src/__tests__/`. Enumerate tools via `SERVER_PACKAGES` + `deriveKeysForTool` from `@iris-mcp/shared`'s `governance-baseline-derivation` against each `packages/<pkg>/dist/tools/index.js` `tools` array (turbo `test dependsOn build`, so the dists exist at test time). Single-source the check logic (a helper imported by BOTH the CLI script and the vitest test) so `pnpm <check>` and the default suite never diverge.

**Why:** Epic 25 (commits `0c4cb01`, `8dc2b7a`). CR 24.0-1 (the Rule #44 `readOnlyHint` cross-check) was routed by the Epic 24 retro to "Story 25.0", but 25.0 is framework-plumbing in `@iris-mcp/shared` — which cannot see leaf-package annotations. The lead moved it to Story 25.1 and landed it in `iris-mcp-all` (which gained its first vitest setup). `validate-prompts` and the doc-rot sync guard landed there for the identical reason. This recurs for every future cross-package check; getting the placement wrong is a circular-import compile failure discovered late. Pairs with Rule #16 (verify the plan's claim — here the retro's "Story 25.0" routing — against dependency-direction reality before trusting it).

**How to apply:** at story creation, when a check needs all-five-packages metadata, spec its home as `iris-mcp-all` (not shared), name the `SERVER_PACKAGES`/`deriveKeysForTool` enumeration pattern, and single-source the logic between the CLI script and the default-suite test.

---

## 46. Live-MCP-Client-over-in-memory-transport smoke for a new protocol capability, including the omitted-optional-field edge

**Context:** The lead's per-story smoke (Rules #22/#26) for a story that adds a new MCP **protocol capability** to the framework — `prompts`, `resources`, completions, or any `initialize`-advertised capability served through the SDK's own request handlers — as opposed to a plain tool or a REST endpoint.

**Rule:** Drive the BUILT `dist/` through a REAL MCP `Client` connected over `InMemoryTransport.createLinkedPair()` (construct the `McpServerBase` from dist, `client.connect()` + `base.server.connect()`), and exercise the ACTUAL protocol surface: `getServerCapabilities()` for the advertise/empty-pack back-compat (Rule #19), `client.listPrompts()`/`listResources()` for advertisement, `client.getPrompt()`/`readResource()` for rendering — NOT an internal `_requestHandlers` accessor or a mocked handler. Crucially, include the **omitted-optional-field edge**: call the capability with an OPTIONAL field entirely absent from the request (e.g. `getPrompt({name})` with no `arguments`), because the MCP wire schema marks those fields optional and a spec-compliant client MAY omit them — this is exactly where SDK arg-parsing contracts bite. Assert the result renders (or fails CLEANLY with a documented `-32602`), never crashes.

**Why:** Epic 25 (Stories 25.0/25.1). The real-`Client` smoke caught TWO SDK-contract defects that mocked/internal-accessor unit tests structurally could not: CR 25.0-1 (a zero-argument prompt REJECTED an omitted `arguments` because the framework always passed an empty `argsSchema` → the SDK `z.object({}).parse(undefined)` fails; MCP marks `arguments` optional) — FIXED; and CR 25.1-6 (the same failure for ALL-optional-argument prompts — `check-system-health`/`recover-stuck-production` et al. — an SDK limitation: `_createRegisteredPrompt` hardcodes `objectFromShape` → a plain `z.object(shape)` that rejects `undefined`, and there is no SDK schema that both ADVERTISES optional args and TOLERATES an omitted top-level `arguments`) — DOCUMENTED. The per-server unit tests used `arguments: {}` and so were blind to the omission edge. Specializes Rule #22 (smoke the built artifact in a real process) to the protocol-capability case, adding the "drive real protocol handlers + probe the omitted-optional-field edge" discipline.

**How to apply:** for a protocol-capability story, the lead smoke constructs the dist server + a real SDK `Client` over in-memory transport; assert empty-pack ⇒ capability absent (Rule #19), non-empty ⇒ advertised + list + get render, and one call with an optional field OMITTED entirely. A capability whose smoke only ever sends fully-populated (or `{}`) args has not tested the edge that breaks in practice.

---

## 47. Verify a spec's "reuse the existing X" claim against source before building on it

**Context:** A story spec (or feature spec) instructs the dev to "reuse the existing X" / "extend the existing X" where X is an INTERNAL codebase artifact — a helper function, a SQL query/handler, a shared module — as the basis for new work (distinct from an IRIS system API, which Rule #16 already covers).

**Rule:** Before building on a "reuse the existing X" spec claim, VERIFY that X actually exists AND has the shape/capability the spec assumes, by reading X's real source. Internal-code "reuse" claims are as fallible as IRIS-API claims — the spec author may have mis-remembered a helper's location, a query's filter set, or a function's signature. If the claim is wrong, correct the spec IN PLACE and flag it for the retro (Rule #42 shape). Do NOT let a downstream story assume a helper/query/module that isn't there or doesn't do what's claimed.

**Why:** Epic 26 hit this TWICE, both caught by source-read/probe before the dependent code shipped:
- **Story 26.0 (AC 26.0.4):** spec §3 said "reuse the **existing** `horologToIso` helper in the interop package" — no such helper exists in `packages/iris-interop-mcp/src/` (grep-confirmed zero `horolog` matches); the resend/preview timestamps arrive pre-formatted as an ODBC string via SQL `SELECT TimeCreated`, so NO conversion helper is needed at all. 26.2 would have imported a nonexistent helper.
- **Story 26.1:** spec §4 said `resendFiltered` should "reuse/extract the message-query SQL already used by the production-messages handler (same filters: item, status, time window)" — but `Interop.cls:MessageTrace` has NO item/status/time-window filters (only `sessionId`/`headerId`, verified by reading `Interop.cls:1009-1090`), so a fresh bounded `MessageTrace`-style query was required, not a literal reuse.

Generalizes Rule #16 (verify IRIS API shapes) and Rule #42 (verify a spec's own claims) from IRIS-system APIs to INTERNAL-codebase "reuse" claims.

**How to apply:**
- Story creation / dev: when a spec says "reuse/extend the existing `<internal X>`", open X's source and confirm it exists with the claimed shape/capability BEFORE coding against it; a wrong claim is corrected in the spec in place + flagged in Dev Notes for the retro.
- Code review: a "reuse X" implementation whose author did not verify X's actual shape (and X turned out different) is a finding — the divergence should have been caught and the spec corrected.

---

## 48. A burn-down "resolved" fix must be live/mutation-verified, not trusted on a green suite

**Context:** A deferred-work ledger burn-down (Rule #37) where a carried item's disposition is **resolved** via a code fix — ESPECIALLY a fix to a dispatcher/handler path that unit tests structurally cannot exercise (a `%request`/CSP-context path, a live-endpoint-only behavior, a resource-cleanup-on-error path).

**Rule:** A burn-down "resolved" fix carries a HIGHER verification bar than a normal story fix: prove it by a **live verification** (exercise the fix on the real surface) OR a **mutation test** (revert the fix → confirm a test goes red → restore). A green DEFAULT suite is NOT sufficient evidence a burn-down fix is genuine — the item was deferred in the first place precisely because it was under-covered, so the suite was already blind to it. A DEFECTIVE "resolved" fix is WORSE than an honest re-deferral: it closes the ledger item while leaving (or introducing) the defect, and the closure hides it from the next gate. Code review of a burn-down MUST live-verify each "resolved" fix that touches an untestable path; a "resolved" outcome with no failing-if-reverted evidence is a HIGH finding.

**Why:** Epic 26 Story 26.4 (the Rule #37 burn-down). CR 23.1-5's FIRST "resolved" fix — `Set tParamCount = +$Get(%request.Data("areas"))` — was itself DEFECTIVE: `%request.Data("areas")` (unsubscripted) holds no value, so it read `+""` = 0, the loop never ran, and the caller's `areas` filter was silently IGNORED on every real GET (`iris_health_check` always sends `?areas=<csv>`) — a regression vs the prior `$Get(%request.Data("areas",1))`. The DEFAULT suite was GREEN (the GET `%request` path isn't unit-testable without a live CSP context). It was caught ONLY because the Opus code review live-verified over HTTP (corrected to `%request.Count("areas")`; confirmed `?areas=journal`→journal-only, comma + repeated-param both work live). By contrast, QA had mutation-verified the CR 22.1-1 Compressor guard (reverted the guard → test went red → restored) — the exact rigor every burn-down code fix should carry. Extends Rule #37 (terminal disposition) with a verification-rigor requirement specific to the "resolved" outcome.

**How to apply:**
- Burn-down story AC: each "resolved" fix names its live/mutation proof (which real-surface check or which revert→red→restore test proves it).
- Lead smoke: live-exercise any dispatcher/endpoint fix a burn-down resolved (the Epic 26 lead smoke drove `/monitor/health?areas=...` live and confirmed the corrected filter).
- Code review of a burn-down: live-verify (do NOT suite-trust) each resolved item on an untestable path; a resolved fix with no failing-if-reverted evidence is HIGH.

---

## 49. Round-trip-to-clean live capstone for diff/sync/promote features — implementation-derived mocked fixtures encode the implementation's bugs

**Context:** Any feature whose contract is a comparison or synchronization invariant between two live sides — an environment diff, a promote/sync/replicate/migrate pipeline, a reconciler — where "correct" means the two sides converge (a promote applied to the reported drift must yield a clean re-diff).

**Rule:** The epic-done gate (Rule #21 shape) MUST include a **LIVE round-trip capstone**: seed known drift on real instances → diff → plan → execute → **re-diff must report CLEAN (zero drift)**. Convergence-to-clean is a *self-validating oracle* that no mocked fixture can provide, because mocked expected values are typically derived FROM the implementation's own output — a green mocked suite can therefore **encode an implementation bug as the expected value** and keep it green forever. The mocked suite tests the implementation against itself; only the round-trip tests it against reality. When the capstone finds a defect, fix it, add a regression test whose expected value comes from the live behavior (Rule #36 shape), **mutation-verify** the regression (revert the fix → red → restore, Rule #48 shape), and re-run the capstone to a clean pass before closing.

**Why:** Epic 27 Story 27.4 (commit `0ee8d2f`). The `iris_env_diff` mocked suite was fully green — ~all fixture expectations had been authored from the implementation's output, which included the buggy `mappingKey` (see Rule #50). Every mocked diff/plan/execute test PASSED while the tool reported spurious drift on every real cross-namespace diff AND promote was structurally unable to converge. The Lead-authorized live capstone (seed → diff → plan → execute → re-diff) caught it as a HIGH: the re-diff was NOT clean. Fixed; cross-namespace regression added + mutation-verified; capstone re-run clean; confirmed again at the retro's post-reload MCP-layer smoke (14 identical mappings across HSCUSTOM/SADEMO, zero spurious drift).

**How to apply:**
- Epic planning for any diff/sync/promote-shaped feature: name the round-trip capstone in the closing story's ACs, with the convergence assertion ("re-diff after execute reports zero drift for the promoted items") explicit.
- The seed must create drift ACROSS genuinely different sides (different namespaces/instances/profiles — Rule #34 shape), not two handles to the same state.
- Code review: a diff/sync feature whose only evidence is a mocked suite — however green — has NOT demonstrated its core invariant; treat a missing round-trip capstone as a HIGH-risk gap. Ask of each fixture: "was this expected value captured from an independent oracle, or from the implementation under test?"
- Pairs with Rule #21 (genuine capstone), Rule #34 (second-namespace smoke), Rule #36 (reference-captured expected values — this rule is its "no reference exists, so build the oracle from convergence" counterpart), Rule #48 (mutation-verify the fix).

---

## 50. A comparison/matching key must contain only item-identity dimensions — never a per-side dimension

**Context:** Constructing the identity key used to MATCH items across two sides — a diff's "same item on both sides?" key, a join/reconcile key, a cross-environment dedup key.

**Rule:** The key must be composed ONLY of dimensions that identify the *item itself*, and must EXCLUDE any dimension that identifies the *side being compared* (namespace, profile name, host, environment name, database path, timestamps). A side dimension embedded in the key makes cross-side matches **structurally impossible** — two genuinely-identical items hash to different keys, so everything degenerates into `onlyInSource`/`onlyInTarget` and nothing ever `differs` or matches `identical`. Side-scoped attributes belong in the compared **VALUE** (where a mismatch correctly reports as `differs`), never in the key. Litmus test per key component: *"would two genuinely-identical items on the two sides differ in this component?"* — if yes, it is a side dimension; remove it from the key.

**Why:** Epic 27 Story 27.4 (commit `0ee8d2f`, HIGH found by the live capstone): `iris_env_diff`'s `mappingKey` was `` `${type}::${namespace}::${name}` `` — but `namespace` is the SIDE (each profile resolves its own default namespace: HSCUSTOM vs SADEMO), so identical mappings across the two profiles NEVER matched: 100+ spurious drift lines on every cross-namespace diff, and `iris_env_promote` could never reach a clean re-diff (promoting created the mapping in the target's namespace, which then STILL keyed differently). Fix: key = `` `${type}::${name}` ``; `mappingValue`/`mappingValuesEqual` were already namespace-exclusive (correct — the value never had the side in it; only the key did). The mocked suite had encoded the buggy key into its expected values (Rule #49). Verified live: post-fix capstone clean; retro MCP smoke shows 14 identical mappings across the two namespaces.

**How to apply:**
- When writing any cross-side matcher, enumerate the key components and run the litmus test on each; document WHY each component is item-identity (a one-line comment on the key builder).
- Code review of diff/join/reconcile code: a key builder that interpolates a namespace/profile/host/env variable is a finding until proven item-identity.
- The symmetric check: the compared VALUE should carry the attributes that may legitimately differ per side; a value that embeds the key (or the side) double-counts.
- Pairs with Rule #49 (the live round-trip is what exposes a side-dimension key) and Rule #29 (the other key-construction rule: reject delimiter injection in user-supplied key slots).

---

## Rules captured: 50
## Epics contributing: 26 (retros 2026-04-21, 2026-04-22, 2026-06-16, 2026-06-17, 2026-06-18, 2026-07-02, 2026-07-03, 2026-07-04, 2026-07-08 ×2, 2026-07-08 Epic-25, 2026-07-09 Epic-26, 2026-07-11 Epic-27, 2026-07-11 Epic-28)

**Audit note (2026-07-11, Epic 28):** Epic 28 (SQL Performance Advisor — the `advise` action on the existing `iris_sql_analyze`, `@iris-mcp/dev`, the "signature feature") was a four-story feature epic on branch `epic28`: 28.0 mandatory Rules #14/#16 live probe (plan markers VERBATIM + `%Dictionary.CompiledIndex`/`CompiledClass` surfaces + workload availability, all on IRIS 2026.1 Build 235U) → 28.1 the tool family's FIRST ObjectScript handler `ExecuteMCPv2.REST.SqlAdvisor` serving `POST /dev/sql/advise-data` (plan text + index-dictionary rows in ONE response) + bootstrap bump → 28.2 the pure-TS `analyzeAdviceData` heuristic engine (5 findings: full-scan/missing-index/stale-stats/unused-index/plan-anomaly) + reference-captured fixtures + the durable `ExecuteMCPv2.Tests.AdvisorFixture` schema → 28.3 the `advise` action wiring + `mutates:{advise:"read"}` + Rule #19 snapshot + Rule #30 docs rollup + Rule #34 live smokes. The retro nominated 4 rule candidates; **the Project Lead judged ZERO worth codifying — Epic 28 EXERCISED the existing rule set rather than surfacing new general patterns** (project-rules.md stays at 50). The three near-miss candidates and why they were rejected: (a) "split producer/consumer identity — derive from the producer's PRIMARY output (plan text) not its SECONDARY/derived output (extracted `tables`)", a real design win that dodged CR 28.1-1/-2 but a specific application of existing discipline, not a class-of-bugs rule; (b) "a probe must validate the spec's proposed DATA SOURCE is LIVE, not just present" — already within Rule #42's scope (verify the spec's own claims live); (c) "`.gitattributes eol=lf` needs a one-time renormalize to convert already-checked-out CRLF files" — a narrow Windows/autocrlf hygiene gotcha (Rule #1 excludes narrow one-offs), recorded in the retro §3/§8 as an action item not a rule. The HEADLINE win: **Story 28.0's mandatory probe empirically corrected a spec heuristic SOURCE before any dependent code shipped** — `%Dictionary.CompiledStorage.ExtentSize`/`CompiledStorageProperty.Selectivity` are declarative/compile-time-only and do NOT reflect live `TuneTable` state on 2026.1 (verified 4 ways incl. post-recompile + direct `SetExtentSize`), so `stale-stats` was re-sourced to the EXPLAIN plan's own `Warning: Table X is not tuned.` line (simpler, zero-extra-call) — a textbook Rule #16/#42 payoff. All 4 stories closed at **0 HIGH shipped**; 4 MED found+fixed+mutation-verified in-epic (28.2 unused-index harmful "drop your primary key" false-positive on system classes where the plan map-name [`...Master`] differs from the dictionary index name + a throw on malformed `indexes` shapes; 28.3 workload per-statement `catch` masking non-`IrisApiError` outages + `topN` clamp hardening); 13 LOW/MED deferred Epic-28-own (28.0: 2, 28.1: 4 incl. CR 28.1-1 `ResolveClassForTable` bare-Catch + CR 28.1-2 `ExtractTables` bitmap-only-plan; 28.2: 4 incl. CR 28.2-1 multi-table JOIN predicate cross-attribution; 28.3: 3), all below the Rule #37 ≥3-consecutive threshold. `BOOTSTRAP_VERSION` `1e2008753853`→`6422caf6ec31` (one clean bump, Story 28.1, the new `SqlAdvisor.cls`; Rule #39 dual-roster updated — `AdvisorDataTest`/`AdvisorFixture` correctly EXCLUDED as test/fixture classes; stable 28.2–28.3). Frozen governance baseline `1e62c5ad5bf7` UNTOUCHED (`gen:governance-baseline:check` exit 0 every checkpoint); the one new key `iris_sql_analyze:advise` is post-foundation read/default-enabled (live 200→201). Tool count UNCHANGED (Rule #31 — `advise` is an ACTION not a tool; package `tools/index.ts` length + `getToolNames()` assertions did not move); dev-mcp tests 503→576. Existing rules exercised and held, not re-codified (Rule #1): Rule #14/#16/#42 (28.0 probe pinned every heuristic source live + corrected the ExtentSize source); Rule #17 (glob-path loads — the 28.0 dev hit and self-corrected the bare-glob trap); Rule #18/#24/#39 (bootstrap regen-only, one bump, dual-roster with test/fixture classes excluded); Rule #19 (four existing actions byte-for-byte + a genuine snapshot gate); Rule #26/#34 (live HTTP endpoint smokes + e2e endpoint→built-engine + SADEMO second namespace + a 2-table join exercising CR 28.2-1 live + the workload source live); Rule #28 (`advise:"read"` mandatory classification even for a read); Rule #29 (input hygiene); Rule #30/#31 (docs rollup with the read/enabled-by-default callout + explicit tool-count-unchanged, action-not-tool); Rule #35 (%UnitTest 13/13 total-vs-expected count-check); Rule #36 (BINDING — every fixture reference-captured from live 2026.1 plans, cited per fixture, reviewer spot-re-captured 2 fixtures byte-for-byte); Rule #38 (`topN` documented as analysis-breadth scan-work); Rule #47 (three spec-reuse corrections verified against source — the stale "read the existing OS handler" instruction, `Tests` plural, `Temp.*` probes); Rule #48 (the two 28.2 MED fixes mutation-verified, not suite-trusted); Rule #49/#50 (Epic-27 retro §8 directive — applied as review lenses; the 28.2 reviewer confirmed the engine's correlation keys are item-identity-clean and the fixtures are independent-oracle-captured). **Epic 27 retro §8 Action #1 (`.gitattributes`) DISCHARGED at kickoff** (added `* text=auto eol=lf` + a one-time LF renormalization of the on-disk `skills/**` in Story 28.1, content-identical — nothing staged — restoring `gen-skills --check` green, `@iris-mcp/all` 49/49). Retro-review gate at kickoff **re-deferred** the carried ledger (Epic-26-own batch + Epic-27-own items — feature epic, DISJOINT surface, no colliding Story X.0 [28.0 is the feature probe], **2nd consecutive re-deferral of the Epic-26-own batch**). **Rule #37 ⚠️: the 3rd re-deferral would occur at Epic 29 → the Epic 29 plan MUST include a dedicated deferred-work burn-down story** (terminal disposition for the Epic-26-own batch + all carried Epic-27/28-own items — re-deferral NOT allowed) — this is the binding directive carried out of the Epic 28 retro. Operational item to Epic 29 kickoff: rebuild+reload the running MCP servers so `iris_sql_analyze:advise` goes live at the MCP layer (mirrors the Epic 26/27 reload pattern; the per-story smokes exercised the built dist + deployed endpoint directly, governance proven by real-`handleToolCall` unit tests). Epic 28 closes 0 HIGH shipped / 0 rework / 0 post-close smoke defects; merged to `feature/feature-wave-1` per the Project Lead's gate decision.

**Audit note (2026-07-11, Epic 27):** Epic 27 (Environment Diff & Promotion — `iris_env_diff` / `iris_env_promote`, the "moat" feature) was a five-story feature epic on branch `epic27`: 27.0 probe + `/dev/doc/hashes` OS endpoint (`EnvSync.cls`, SHA-256 via `$SYSTEM.Encryption.SHAHash` — `$ZCONVERT("O","HEX")` throws on this instance) + `ToolContext.resolveProfileClient` → 27.1 `iris_env_diff` mappings/SDS/webapps/config domains + credential redaction + per-domain isolation → 27.2 `iris_env_promote:plan` (pure transform, `planHash`, NO-DELETE type-enforced via closed `PlanOperation` union, `onlyInTarget` → warnings) → 27.3 `iris_env_promote:execute` behind 4 refuse-before-any-write gates (confirm / steps-allowlist / plan-hash+steps-vs-diff anti-tamper / target-profile governance via `effective()`) → 27.4 docs rollup + `promote-environment-change` prompt + the Lead-authorized LIVE CAPSTONE. First epic whose tools operate on TWO live profiles at once (`default`→HSCUSTOM, `sademo`→SADEMO, single instance — the Epic 26 retro-settled prereq). The retro nominated 3 rule candidates; the Project Lead delegated the general-pattern call, and 2 were codified (Rule #49 — round-trip-to-clean live capstone for diff/sync/promote features, mocked implementation-derived fixtures encode the implementation's bugs; Rule #50 — a comparison key must contain only item-identity dimensions, never a per-side dimension) while the 3rd (per-profile namespace routing) was correctly REJECTED as a narrow one-off already covered by the Rule #34/#40 smoke discipline that caught it. The headline: **the 27.4 live capstone caught a HIGH the fully-green mocked suite had ENCODED AS CORRECT** — `mappingKey` included the per-side namespace, so identical mappings across differently-named namespaces never matched (spurious drift everywhere; promote structurally unable to converge); the mocked expectations were authored from the buggy output, so the suite certified the bug. Fixed (`type::name`), cross-namespace regression + mutation-verify, capstone re-run CLEAN. All 5 stories closed at **0 HIGH shipped**; 2 HIGH found+fixed in-epic (CR 27.1-1 config falsy-value `!targetEntry` truthiness → `!targetMap.has(key)`; the capstone mappings-key bug); 3 Lead-directed reworks at story-boundary smokes (per-profile namespace routing after `namespace:""` output; per-domain error isolation + `documents` made OPT-IN with required `spec` (Rule #38 shape) after a bare diff aborted on a non-interop namespace; broadened `isCredentialSetting` redaction list); 6 security patches from the Opus review of 27.3's execute gates (Gate 3b plan-steps-vs-diff anti-tamper ×3 layers, credential error-path scrub, fail-closed empty-doc, Gate 3c wrong-instance). `BOOTSTRAP_VERSION` `e5c18edd00c0`→`1e2008753853` (27.0, the new `EnvSync.cls`; Rule #39 dual-roster updated; stable 27.1–27.4). Frozen governance baseline `1e62c5ad5bf7` UNTOUCHED (`gen:governance-baseline:check` exit 0 every checkpoint); the new keys are post-foundation: `iris_env_diff`/`iris_env_promote:plan` read/default-enabled, `iris_env_promote:execute` write/DEFAULT-DISABLED and deliberately NOT `defaultEnabled` (Rule #32 deliberately not invoked — promotion is not a recovery tool). Tool counts dev 26→28 / suite 102→104; prompts 10→11. Existing rules exercised and held, not re-codified (Rule #1): Rule #16/#42 (27.0 probe pinned the hash surface live before dependent stories); Rule #17 (glob-path loads); Rule #18/#24/#39 (bootstrap regen-only, one bump, dual-roster for the new class); Rule #19 (strictly additive); Rule #21 (the capstone named as the epic gate — #49 extends it with the convergence oracle); Rule #23/#25/#28 (frozen baseline untouched; per-action `mutates:{plan:read, execute:write}`); Rule #26/#34 (live gated-write refusals + genuinely-different second namespace — the capstone seeded drift across HSCUSTOM/SADEMO); Rule #29 (#50 is its key-construction sibling); Rule #30/#31 (docs rollup states execute's default-disabled state across all surfaces); Rule #36 (post-fix regression expected values captured from live behavior); Rule #38 (documents domain requires `spec`, bare `*` refused without `allowWide`); Rule #47 (27.3 verified the reused `env-diff.ts` fetchers against source); Rule #48 (the mappings fix was mutation-verified, not suite-trusted). **Epic 26 retro Action #1 DISCHARGED at this retro:** MCP servers rebuilt+reloaded 2026-07-11; the live MCP-layer smoke confirmed both profiles resolve, `iris_env_promote:execute=false` on both, cross-namespace diff clean of spurious mapping drift (14 identical), `plan` emits `onlyInTarget` as a warning with no delete op, and `execute` refused at the governance boundary with ZERO writes reaching IRIS. Retro-review gate at kickoff re-deferred the 10 Epic-26-own LOW items (feature epic, 1st re-defer, below the Rule #37 ≥3 threshold, no colliding Story X.0); the ledger carries those 10 + Epic-27's own LOW items into the Epic 28 retro-review gate (a 2nd re-defer at Epic 28 is permitted; a 3rd at Epic 29 trips the threshold → plan the burn-down there). Epic 27 closes 0 HIGH shipped / 3 story-boundary reworks (all caught by lead smokes, none post-close) / 0 post-close smoke defects; merged to `feature/feature-wave-1` per the Project Lead's gate decision.

**Audit note (2026-07-09, Epic 26):** Epic 26 (Interop Message Resend / Replay — `iris_message_resend`) was a five-story feature epic on branch `epic26`: 26.0 mandatory Rule #16 resend-API probe → 26.1 ObjectScript handlers (`MessageResend.cls`) + 6 pre-mutation guards + %UnitTest + bootstrap bump → 26.2 `iris_message_resend` TS tool + governance (interop 21→22; per-action `mutates:{preview:read, resend:write, resendFiltered:write}`, writes DEFAULT-DISABLED, deliberately NOT `defaultEnabled`) → 26.3 docs rollup + gated `resend-failed-messages` prompt (prompts 9→10) + live resend smokes → **26.4 the Rule #37 dedicated deferred-work ledger BURN-DOWN** (16 carried-open items → ZERO carried-open: 11 resolved / 4 closed-by-decision / 1 closed-with-evidence / 0 re-deferred). The retro-review gate created Story 26.4 per the BINDING Project-Lead decision (deferred-work.md) — the Epic-22-own batch had been re-deferred by Epics 23/24/25 (3 consecutive, tripping Rule #37's ≥3 threshold). The retro nominated 2 rule candidates; the Project Lead confirmed BOTH pass the general-pattern bar (Rule #47 — verify a spec's "reuse the existing X" INTERNAL-code claim against source before building; Rule #48 — a burn-down "resolved" fix must be live/mutation-verified, not trusted on a green suite). Both generalize: #47 recurs on every feature spec that says "reuse the existing helper/query/module" (Epic 26 hit it TWICE — the nonexistent `horologToIso` helper [26.0] and the filterless `MessageTrace` query [26.1], both caught by source-read before the dependent code shipped); #48 recurs on every burn-down "resolved" fix touching an untestable path (CR 23.1-5's first fix was itself defective — `%request.Data("areas")` mis-read the GET filter to 0, silently ignoring it on every real call, GREEN suite, caught ONLY by the Opus review's live-HTTP verify → corrected to `%request.Count("areas")`). All 5 stories closed at **0 HIGH shipped / 0 rework loops**; 1 HIGH found+fixed INSIDE the 26.4 review (CR 26.4-R1, the defective CR 23.1-5 fix). Fixes applied inline across the epic: 26.1 (6 LOW deferred — no guard bypass, folded into 26.2/hardening); 26.2 (1 patch — leading-zero header-ID rejection; 1 LOW deferred); 26.3 (3 LOW patched — dry-run example count, stale meta-package README counts reconciled interop 22/ops 21/102 tools, root-README duplication-hazard); 26.4 (1 HIGH fixed + 3 LOW deferred + 3 dismissed). `BOOTSTRAP_VERSION` `13b4b5f003ab`→`1f3afba4ac52` (26.1)→`f16c695aa9ef` (26.4 dev)→`e5c18edd00c0` (26.4 review Health fix); Rule #39 dual-roster correctly updated for the NEW `MessageResend.cls` (both `gen-bootstrap.mjs` array + `bootstrap.test.ts` roster 25→26). Frozen governance baseline `1e62c5ad5bf7` UNTOUCHED throughout (`gen:governance-baseline:check` exit 0 every checkpoint; the 3 new resend keys are POST-foundation, live 194→197). Lead LIVE smokes each story (Rule #26): 26.1 five destructive-path refusals NO-WRITE; 26.3 real resend `82651→82658` verified via `iris_production_messages` (SessionId preserved) + refusals NO-WRITE (before/after counts) + Rule #34 second-namespace SADEMO (guards namespace-generic) + scratch cleanup to zero; 26.4 corrected `/monitor/health` area-filter live. HSCUSTOM `SessionAgent.Sample.Production` used as the disposable scratch target (started/stopped, scratch headers deleted — per the pre-recorded authorization). Existing rules exercised and held, not re-codified (Rule #1): Rule #16/#42 (26.0 pinned `Ens.MessageHeader.ResendDuplicatedMessage` from `irislib` source + live probe, rejected the spec's "resubmit variants" candidate; #47 is the internal-code specialization); Rule #17 (glob-path `iris_doc_load` throughout); Rule #18/#24/#39 (bootstrap regen-only, 3 bumps, dual-roster for the new class); Rule #19 (strictly additive — interop 21→22, no existing tool/output changed); Rule #21 (all guard/governance/prompt/doc-rot tests in the DEFAULT suite); Rule #23/#25/#28 (frozen baseline untouched, `:check` exit 0, bare generator never run, per-action `mutates` map = the required classification incl. `preview:read`); Rule #26/#34 (live destructive-path refusals + second-namespace smoke); Rule #29 (numeric-header-ID + item/from-required input hygiene guards); Rule #30/#31 (docs rollup across all four surfaces + meta-package README with the default-disabled callout; `iris_message_resend` is a PACKAGE tool so package counts moved, the prompt is a protocol capability so no tool-count change); Rule #33 (SanitizeError — the pinned `<Ens>...` errors are angle-bracket, safe, no caret-strip); Rule #35 (%UnitTest total-vs-expected count-check on every OS test run — 13→14, 15/15, 10/10); Rule #37 (the burn-down itself — zero re-deferred, disposition table mirrored into the ledger; #48 adds the resolved-fix verification-rigor requirement); Rule #44/#45 (the `readOnlyHint` cross-check + prose-count assertion live in `iris-mcp-all`; the new `preview:read`/`readOnlyHint:false` divergence is legitimate — post-foundation keys sit outside the frozen-baseline-scoped Rule #44 check); Rule #46 (the 26.4 prompt-plumbing fixes CR 25.0-4/5/25.1-6 exercised the protocol-capability surface). **Retro action item (operational):** rebuild + reload the running MCP servers so `iris_message_resend` + the `resend-failed-messages` prompt go live at the MCP layer (mirrors the Epic 20 retro Action #2 pattern) — the governance-disabled-write LIVE smoke was covered by 26.2's real-`handleToolCall` unit tests, not a live MCP call, pending this reload. Epic 27 (`iris_env_diff`/`iris_env_promote`) prereq (≥2 reachable profiles) confirmed SETTLED at this retro — two profiles on the single dev instance with different default namespaces is the accepted path, no kickoff blocker. Epic 26 closes 0 HIGH / 0 rework / 0 post-close smoke defects; the deferred-work ledger stands at ZERO carried-open (only Epic-26-own CR 26.1-*/26.2-1/26.3-*/26.4-* remain) for the Epic 27 retro-review gate.

**Audit note (2026-07-08, Epic 25):** Epic 25 (MCP Prompts Capability & Agent Skills Pack) was a three-story TS/content-only feature epic (25.0 framework `prompts` capability on `McpServerBase` + back-compat capability snapshot → 25.1 the 9 stakeholder-approved v1 prompts per owning server + `gen-skills.mjs`/`validate-prompts.mjs` + the CR 24.0-1 `readOnlyHint` cross-check → 25.2 docs rollup + live-IRIS `deploy-and-test-class` client smoke). No ObjectScript, no bootstrap bump, no new tool/governance key (Rule #31 — prompts are a protocol CAPABILITY, not tools), frozen baseline `1e62c5ad5bf7` untouched throughout (`gen:governance-baseline:check` exit 0 every checkpoint; tool counts unchanged everywhere — dev 26/admin 26/interop 21/ops 21/data 7 = 101 package / 106 advertised). The retro nominated 2 rule candidates; the Project Lead confirmed both pass the general-pattern bar (Rule #45 — cross-package validation tests live in the all-packages aggregator, never `@iris-mcp/shared`; Rule #46 — live-MCP-Client-over-in-memory-transport smoke for a new protocol capability, including the omitted-optional-field edge). Both generalize beyond trigger: #45 recurs for every future cross-package catalog/lockstep check (CR 24.0-1's `readOnlyHint` cross-check + `validate-prompts` + the doc-rot guard ALL landed in `iris-mcp-all` because shared can't import leaf packages — circular; `iris-mcp-all` gained its first vitest setup this epic); #46 recurs for every future protocol capability (`resources`, completions, Epic-26+ surfaces) — the real-`Client` smoke caught CR 25.0-1 (zero-arg prompt rejected an omitted `arguments`, FIXED) and CR 25.1-6 (all-optional-arg prompts reject an omitted `arguments` — an SDK limitation with no schema that both advertises optional args and tolerates omission, DOCUMENTED), both invisible to the unit tests (which used `arguments:{}`). All three stories closed at **0 HIGH, 0 rework**; fixes applied inline: 2 MED + 1 LOW-doc (25.0 — CR 25.0-1 no-arg omission, CR 25.0-2 `build(args: Record<string,string|undefined>)`, CR 25.0-3 File List), 1 MED + 1 LOW-MED (25.1 — CR 25.1-1 cross-server tool reference in `check-system-health` [ops prompt naming an admin tool as if ops-local], CR 25.1-2 cross-check test assertion strengthened), 2 MED (25.2 — CR-25.2-A doc-rot guard hardened with a cross-attribution check, CR-25.2-B File List reconciliation). The CR 24.0-1 `readOnlyHint` cross-check (from Epic 24's retro) landed and HELD GENUINE: mutation-tested, surfaced 15 divergences, all verified per-key as legitimate multi-action-tool cases (7 route through a single `ctx.http.post` distinguished only by an `action` body field, requiring OS-side cross-check) → **0 reclassifications** (no new latent misclassifications). **Retro-review gate placement correction (Rule #16 shape):** the Epic 24 retro routed CR 24.0-1 to "Story 25.0", but it moved to 25.1 at the gate because the cross-check needs leaf-package annotations that shared cannot import — the exact situation that motivated Rule #45. Existing rules exercised and held, not re-codified (Rule #1): Rule #16 (SDK `registerPrompt`/`_createRegisteredPrompt`/zod-compat source read to diagnose CR 25.1-6; the CR-24.0-1 placement correction); Rule #19 (empty-pack capability snapshot deep-equals today's — the SDK advertises `prompts` only when a prompt is registered; `toEqual` mechanical proof, not prose); Rule #21 (validate-prompts + readOnlyHint + doc-rot + safety-invariant tests all in the DEFAULT suite, not `*.integration.test.ts`); Rule #22/#26 (built-dist real-`Client` smokes each story — #46 specializes this); Rule #23/#25 (frozen baseline git-clean, `:check` exit 0, bare generator never run); Rule #28 (prompts carry no `mutates`/governance key — they are not tools); Rule #30/#31 (docs rollup across README + 5 per-server READMEs + CHANGELOG + tool_support.md, framed as a framework SURFACE with tool counts asserted unchanged; #45 is the cross-package-test analogue of #31's counting discipline); Rule #35 (the live `deploy-and-test-class` smoke exercised the total-vs-expected count check — `total:2` matched, confirming a genuine not-partial run); Rule #37 (the deferred ledger — CR 25.0-4/5, 25.1-3/4/6, 25.2-1 [all LOW] + the 10 re-deferred older LOW — carries into Epic 26, which is now the flagged burn-down candidate: the Epic-22-own batch has been re-deferred by Epics 23/24/25, the 3rd consecutive matching the accepted 19/20/21 pattern). Retro-review gate at kickoff folded CR 24.0-1 into Story 25.1 (not a separate X.0 — the 25.0 slot is a feature story) and re-deferred the 10 older LOW. Epic 25 closes 0 HIGH / 0 rework / 0 post-close smoke defects; the ledger carries into the Epic 26 retro-review gate (burn-down).
