...
**Epics 23–29 — Feature Differentiation Wave 1 (added 2026-07-07)**

Sourced from the feature-differentiation research and specs ([research/feature-differentiation-research-2026-07-07.md](./research/feature-differentiation-research-2026-07-07.md), [research/feature-specs/](./research/feature-specs/) 01–07); see [sprint-change-proposal-2026-07-07.md](./sprint-change-proposal-2026-07-07.md). All seven are strictly additive under the Epics 14–17 backward-compatibility release gate.

- FR131: Operator can obtain a composite health verdict for an IRIS instance in ONE call via a new read tool `iris_health_check` on `@iris-mcp/ops`: structured `healthy`/`warning`/`critical` verdict with per-area findings (system, memory, databases, journal, mirror, locks, license, ECP, alerts, interop), documented overridable thresholds, raw probe values always returned alongside interpretations, and per-area isolation (a failing probe yields an `error` finding capped at `warning`, and non-configured areas report `notApplicable` without affecting the verdict). Classified `mutates: "read"`, enabled by default. Binding spec: feature-specs/01.
- FR132: Operator can activate a one-word safety preset via `IRIS_GOVERNANCE_PRESET` (`read-only` | `full`): `read-only` blocks every write-classified action across the suite — including grandfathered frozen-baseline writes (classified via a new generated `baseline-classifications` artifact; the frozen baseline file itself is never modified) and Epic-20-style `defaultEnabled` writes — while explicit `IRIS_GOVERNANCE` keys still override at both global and profile layers (cascade: explicit > preset > default seed). Denial errors carry `presetApplied`; `iris_server_profiles` and the governance resource report the active preset; an unknown preset value fails fast at startup. Additionally, `IRIS_SQL_MAX_ROWS` and `IRIS_SQL_TIMEOUT` cap `iris_sql_execute` (clamped values annotated in the response). Unset preset and unset caps are byte-for-byte today's behavior. Binding spec: feature-specs/02.
- FR133: AI client can discover packaged expert workflows via the MCP `prompts` capability (`prompts/list` / `prompts/get`) on all 5 servers — v1 ships 9 parameterized workflow prompts (system health, slow-query diagnosis, message-flow tracing, environment provisioning, security-posture audit, ObjectScript review, deploy-and-test, production-recovery escalation [recover first, `clean` last resort, `killAppData` only with explicit data-loss acceptance], safe external freeze/thaw backup [thaw always, even on failure]; list stakeholder-approved 2026-07-07; two more gated prompts ship with their features — message resend with FR134, environment promotion with FR135) naming exact tool sequences with safety notes — plus a generated installable `skills/` directory mirroring the same content for Claude Code/Copilot skill loaders. A CI validation script asserts every tool name referenced in prompts/skills exists in the live catalog. Prompts are not tools: no governance keys, no tool-count changes; servers with no prompts registered advertise no `prompts` capability (back-compat). Binding spec: feature-specs/03.
- FR134: Integration engineer can resend Interoperability messages via a new tool `iris_message_resend` on `@iris-mcp/interop`: `preview` (read — per-header resendability verdicts, never mutates), `resend` (by header-ID list, 1–100, per-header results with new-header linkage, partial failures continue), and `resendFiltered` (bounded batch: required item + time window ≤ 7 days, status filter, `maxMessages` hard cap 500 refused-not-truncated, `dryRun` default true, execution requires `dryRun:false` AND `confirm:true`). All guards enforced server-side before any mutation; refusals mutate nothing. Resend duplicates downstream data flow, so `resend`/`resendFiltered` are classified `mutates: "write"` and **default-disabled** (deliberately not `defaultEnabled`); the exact `Ens.MessageHeader` resend API is pinned by a mandatory live probe story before implementation. Binding spec: feature-specs/04.
- FR135: Operator can detect cross-environment drift and promote changes between two configured server profiles: `iris_env_diff` (read) compares documents (source-content SHA-256 via a new hash endpoint; package spec required), namespace mappings, System Default Settings (credential-ish values redacted), web apps, and config keys, returning `onlyInSource`/`onlyInTarget`/`differs`/`identical` buckets; `iris_env_promote` turns a diff into an ordered dependency-aware plan (`plan` = read; `onlyInTarget` items become warnings, never deletions) and executes ONLY allowlisted plan steps (`execute` = write, **default-disabled**) gated by `confirm:true`, a plan-content-hash freshness check, the target profile's own governance policy, and halt-on-first-error. The shared framework gains `resolveProfileClient` for second-profile access (additive). Binding spec: feature-specs/05.
- FR136: Developer can request advisory SQL performance findings via a new `advise` action on `iris_sql_analyze`: heuristics over plan text, index dictionary, and tuning metadata (gathered by a new consolidated ObjectScript endpoint) produce `full-scan`, `missing-index` (with suggested DDL, never for system schemas), `stale-stats`, `unused-index`, and `plan-anomaly` findings — each with cited evidence, plan excerpt, and a confidence label; unrecognized plan formats return no findings with an explicit note (never crash or guess); strictly advisory (no write actions in v1). Heuristic fixtures are reference-captured from live plan output (Rule #36 discipline). `advise` is classified `read`, enabled by default; the four existing actions are byte-for-byte unchanged. Binding spec: feature-specs/06.
- FR137: Operator can enable a structured, secrets-free JSONL audit trail of every MCP tool call via `IRIS_AUDIT_LOG` (+ `IRIS_AUDIT_LOG_MAX_MB` rotation, `IRIS_AUDIT_LOG_PARAMS` default-off parameter capture): each entry records timestamp, per-process session UUID + monotonic sequence, server package, tool, action, profile, namespace, outcome (`ok`/`error`/`denied` with governance `denyReason` and `presetApplied`), and duration. Mandatory recursive redaction (password/secret/token/credential key families) runs before anything is queued; audit writes never fail or slow a tool call (post-startup sink failures degrade with a dropped-entries counter; an unwritable path fails fast at startup). Audit logging is server configuration, not a tool — deliberately not bypassable via `IRIS_GOVERNANCE`. Unset ⇒ mechanically-proven no-op. Binding spec: feature-specs/07.

**Epic 30 — Tool Visibility Presets (added 2026-07-12)**

Sourced from the tool-count analysis against the 2026-04-05 MCP best-practices research (5–15 tools per server; performance cliff ~20 — the suite had drifted to 29/27 runtime tools on dev/admin) and the approved spec ([research/feature-specs/11-tool-visibility-presets.md](./research/feature-specs/11-tool-visibility-presets.md)); see [sprint-change-proposal-2026-07-12.md](./sprint-change-proposal-2026-07-12.md). Strictly additive under the standing back-compat release gate.

- FR138: Operator can constrain which tools each MCP server ADVERTISES via a new advertise-time visibility layer, orthogonal to call-time governance: `IRIS_TOOLS_PRESET` (`full` = today's behavior, default; `core` = the everyday ≤13-runtime-tool subset per server, tuned for small/local models; `developer` = persona filter hiding security/enterprise administration) plus `IRIS_TOOLS_DISABLE`/`IRIS_TOOLS_ENABLE` comma lists with trailing-`*` wildcards (resolution `ENABLE > DISABLE > preset > default-visible`). A hidden tool is never registered with the MCP SDK — absent from `tools/list`, calling it returns the standard unknown-tool error, zero context cost. Granularity is per-TOOL only (per-action safety remains governance's job); `iris_server_profiles` is reserved always-visible and reports the active preset with visible/hidden counts (never hidden names); the governance report/resource omit hidden tools' keys so the agent's view stays self-consistent. Preset rosters are explicit per-package include/exclude dispositions enforced at construction (a new tool without a disposition throws — the Rule #28 `mutates` discipline applied to visibility) with pair co-visibility guards (`iris_env_diff`/`iris_env_promote`). Unknown preset values fail fast at startup; unknown tool names warn only (the env block is shared across all 5 servers). Unset env vars ⇒ byte-for-byte today's `tools/list` on every server (mechanical Rule #19 proof). Approved rosters: spec §2.5 (2026-07-12). Binding spec: feature-specs/11.

**Epics 31–33 — Connection & Configuration Management Wave (added 2026-07-25)**

Sourced from the Server Manager / MCP configuration research ([research/technical-iris-server-manager-mcp-connections-research-2026-07-25.md](./research/technical-iris-server-manager-mcp-connections-research-2026-07-25.md) — the binding spec for all three). Three independently shippable features; all strictly additive under the standing back-compat release gate; none adds an MCP tool or governance key (tool counts unchanged everywhere).

- FR139: Developer can source IRIS connection profiles from InterSystems Server Manager definitions instead of embedding them in MCP client config: opt-in `IRIS_SERVER_MANAGER` (`off` default | `auto` | `required`) reads `intersystems.servers` (JSONC, via `jsonc-parser`) from VS Code settings files across product variants (Code, Insiders, VSCodium, Cursor) and scopes (workspace overrides user; `IRIS_SM_SETTINGS_PATHS` escape hatch; `IRIS_SM_SERVERS` allow-list), mapping `webServer` definitions into the existing profile registry AFTER env profiles (explicit `IRIS_PROFILES`/`IRIS_*` config always wins name collisions, logged). Passwords are NEVER read from VS Code SecretStorage (verified unreachable outside VS Code by design); each Server-Manager-sourced profile completes credentials via an ordered chain: explicit env → suite-owned OS keychain entry (service `iris-mcp`, managed by a new `iris-mcp-credentials` CLI: `set`/`delete`/`list`/`test`; `@napi-rs/keyring` as optionalDependency with graceful skip) → configurable `IRIS_CREDENTIAL_HELPER` command (git-credential-style, stdout = password) → excluded-with-named-remediation (`required` escalates to startup failure). Profiles carry redaction-safe `source` provenance (`iris_server_profiles` allow-list + audit `profileSource`); governance and session isolation join on profile name, unchanged. A separate optional VS Code broker extension (`iris-mcp-launcher`) registers a `McpServerDefinitionProvider` that resolves full credentials from the Server Manager authentication provider (`intersystems-server-credentials`, password = `session.accessToken`) at server start and spawns the suite with synthesized `IRIS_*` env — zero secrets in any config file, zero npm-package changes required for that path. Unset `IRIS_SERVER_MANAGER` ⇒ byte-for-byte today's profile resolution (mechanical Rule #19 proof). Binding spec: research doc Feature 1.
- FR140: Operator can supply governance policy via a shared file: new `IRIS_GOVERNANCE_FILE` env var points at a JSON file validated by the SAME `parseGovernanceConfig` rules (fail-fast naming file + var; never silently permissive), merged into the effective cascade as explicit `IRIS_GOVERNANCE` env > file > `IRIS_GOVERNANCE_PRESET` seed > defaults, with per-key `configSource` attribution in `iris_server_profiles` and the governance resource. Because a static file path is the only fully agent-agnostic contract across heterogeneous client config formats (JSON/JSONC/TOML, divergent env expansion), one file drives identical effective policy across every MCP client. Managed by a new `iris-mcp-governance` CLI (`validate`/`get`/`set`/`unset` [±`--profile`]/`preset`/`effective`/`diff` — single-sourced with the shared engine, atomic writes with post-write validation) and later by a governance UI view in the `iris-mcp-launcher` extension (same engine, diff preview, effective-policy pane). Enforcement remains solely in the servers at the existing `dispatchToolCall` gate; no hot-reload in v1 (restart semantics, documented). Unset ⇒ mechanically-proven no-op. Binding spec: research doc Feature 2.
- FR141: Operator can wire and manage the suite across MCP clients from one surface: new `@iris-mcp/client-config` package (not a server-runtime dependency) with a declarative 13-client adapter registry — Claude Code, Claude Desktop, Cursor, VS Code (Copilot), Cline, Roo Code, Windsurf, OpenAI Codex CLI, Gemini CLI, Zed, Goose, Kimi CLI, Kimi Code — capturing per-client format (JSON/JSONC/TOML/YAML), root key (`mcpServers`/`servers`/`mcp_servers`/`context_servers`/`extensions`), per-OS paths, scopes, env-expansion syntax, disable capability, and restart hints. Capabilities: client detection; per-client × per-server **enable/disable** (native `disabled`/`enabled` flags where clients have them, else byte-preserving stash-and-remove via manager state); format-preserving writes (surgical `jsonc-parser` edits; TOML splices bounded to owned tables; comment-preserving YAML) under a universal safety protocol (validate → timestamped backup → edit → re-parse → auto-restore → diff-preview-confirm); entry synthesis in four env modes (`server-manager` [FR139], `env-reference` [client-syntax-aware], `governance-file` [FR140], `explicit` — which requires a typed confirmation before any literal secret); the manager only ever modifies entries it owns (third-party entries read-only, never logged). Surfaces: `iris-mcp-clients` CLI (`detect`/`status`/`apply`/`enable`/`disable`/`remove`/`diff`/`doctor`) and an extension "MCP Clients" view with a **user-selectable client roster** and per-server toggle matrix. Pi is documented as not-MCP-capable (excluded with rationale); JetBrains AI Assistant and Kilo Code are roadmap adapters pending config-surface verification. Binding spec: research doc Feature 3 addendum.

**Epic 34 — `iris_execute_classmethod` Fidelity (added 2026-08-14)**

Sourced from live bug report [docs/bugs-2026-08-14.md](../../docs/bugs-2026-08-14.md) (three reproductions of the Write-output envelope-corruption defect) plus stakeholder request; see [sprint-change-proposal-2026-08-14.md](./sprint-change-proposal-2026-08-14.md). Strictly additive under the standing back-compat release gate; no new tool, no governance key, no tool-count change.

- FR142: Developer can call class methods whose execution writes to the current device — the output is captured server-side (same null-device I/O redirect the `/command` endpoint already uses) and returned as an additive `output` response field, instead of corrupting the JSON envelope as today; can pass `ByRef`/`Output` arguments via explicit marker objects in `args` (`{"byRef": true, "value": <optional initial>}`; omitted `value` = undefined, `Output`-style) with post-call values returned in an additive `byRefValues` field keyed by zero-based argument index — completing FR38's output-parameter clause; and can pass up to 20 positional arguments (raised from 10; a 21st is rejected with a clear error). Plain-scalar-args calls return unchanged `returnValue`/`argCount` plus the new additive fields (mechanical Rule #19 pin); non-marker object arguments are rejected with a validation error (object args are undocumented/unsupported today, so this is non-breaking).
