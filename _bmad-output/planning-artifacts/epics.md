...
- **AC 21.0.2** — Loader implements the proposal §6.1 data contract: embedded-SQL over `Ens.MessageHeader` selecting the 12 columns (ID, Invocation, MessageBodyClassName, SessionId, SourceConfigName, TargetConfigName, ReturnQueueName, CorrespondingMessageId, TimeCreated, Type, IsError, ErrorStatus), **NULL-safe trace filter** `(MessageBodyClassName IS NULL OR MessageBodyClassName <> 'HS.Util.Trace.Request')`, `ORDER BY TimeCreated, ID` with fallback `ORDER BY ID`, `maxRows` cap (default 2000, max 10000) with `truncated` flag, `Type` normalized to `"Request"`/`"Response"`/`""` via the `$$$eMessageType*` macros. Invalid/non-positive sessionId → empty result (not an error). Verify column/enum shapes against `irislib` before implementation (Rule #2/#16).
