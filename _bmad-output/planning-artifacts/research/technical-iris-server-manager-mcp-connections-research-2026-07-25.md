...
**Decision marker (Story 36.3, ledger `33-5-L4`, added 2026-09-11):** the `shareable` flag on a `ScopeDef` (§3.2's `ClientAdapter` interface) is a STRUCTURAL classification of the scope itself — project-scope files are the ones a team commits/shares; user-scope files are private to one machine and one person. It is NOT a per-write guarantee that no secret ever lands in that file. Every v1 adapter's project scope (Cursor, VS Code, Roo Code, Gemini CLI, Zed, Kimi Code, and Codex CLI) is uniformly `shareable: true` for this structural reason — Codex is not a special case and was never meant to be. The actual safeguard against inlining a credential into a shareable file is orthogonal (§3.3 point 4, the "explicit" env mode): a literal `IRIS_PASSWORD` requires a typed confirmation before writing and marks the entry `contains-secret`, regardless of which scope it lands in. Probed 2026-09-11 against `packages/client-config/src/adapters.ts`'s `CLIENT_ADAPTERS["codex"].scopes` (project scope: `shareable: true`) — the code's pin is CORRECT and this marker makes the spec's prior silence on the point explicit; no code change was needed.
