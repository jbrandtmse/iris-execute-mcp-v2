...
- **Queue pairing:** primary — Response whose `CorrespondingMessageId` = Request ID; fallback — equal `ReturnQueueName` + reversed endpoints + empty CorrId. Unmatched request → warning. **Never reclassify** a queued pair to sync (even with empty ReturnQueueName). **[I3]** (added 2026-07-02, stakeholder decision at Story 21.1 creation, resolves CR 21.0-1): the `ReturnQueueName` fallback fires only when BOTH values are **non-empty** and equal — empty==empty does NOT pair; a one-way async request stays unpaired-with-warning instead of silently false-pairing with a later CorrId-less response-typed row. Deviation from reference behavior, which paired on trivially-equal empty strings.
