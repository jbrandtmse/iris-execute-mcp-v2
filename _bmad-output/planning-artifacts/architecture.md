...
### Tool Visibility Presets (Epic 30 — added 2026-07-12)

Added via [sprint-change-proposal-2026-07-12.md](./sprint-change-proposal-2026-07-12.md) (FR138; binding spec [research/feature-specs/11-tool-visibility-presets.md](./research/feature-specs/11-tool-visibility-presets.md), rosters approved 2026-07-12). Motivation: the suite drifted past its own researched 5–15 tools-per-server window (dev 29 / admin 27 runtime tools; the research documents a selection-accuracy cliff at ~20), taxing weak clients in tool-selection accuracy and per-turn context tokens. TS-only; no new tools, no governance keys, no bootstrap change.

**I1 — Advertise-time visibility layer in FRONT of the D5 call-time governance gate.** A new env family (`IRIS_TOOLS_PRESET` `full`|`core`|`developer`, plus `IRIS_TOOLS_DISABLE`/`IRIS_TOOLS_ENABLE` comma lists with trailing-`*` wildcards; resolution `ENABLE > DISABLE > preset > default-visible`) filters `options.tools` at the `McpServerBase` constructor registration loop — the same choke point that injects the `server` param — so a hidden tool never enters the SDK registry: absent from `tools/list`, uncallable (standard SDK unknown-tool error), invisible to governance key derivation. The two layers are deliberately orthogonal: visibility is per-TOOL ergonomics ("does the agent know this exists?"), governance stays per-ACTION safety ("is this call allowed?") — read-only *safety* remains `IRIS_GOVERNANCE_PRESET`'s job, and `IRIS_GOVERNANCE` keys naming hidden tools are legal and inert. The D7 fail-fast style applies to a malformed preset value and to a literal disable of the reserved discovery tool; unknown tool names only WARN (the env block is shared across all 5 servers). `iris_server_profiles` (E1) is reserved always-visible and reports `toolVisibility` (preset + visible/hidden counts, never hidden names — invisible means invisible); the E1 governance report and D6 resource omit hidden tools' keys so the agent's view stays self-consistent. Per-profile visibility is impossible by protocol (one `tools/list` per server process) — visibility env vars are process-global, a deliberate contrast with `IRIS_GOVERNANCE`'s profile layer. Unset env ⇒ byte-for-byte today's advertisement (Rule #19 snapshots, all 5 servers). Runtime toggling via the SDK's `RegisteredTool.enable()/disable()` + `listChanged` is explicitly deferred — config is static at startup; the handles `registerTool` returns (currently discarded) are the future lever if ever wanted.

**I2 — Package-owned preset rosters with registration-time coverage enforcement.** Each package declares explicit per-preset `include`/`exclude` dispositions (`src/tools/presets.ts`, passed as `toolPresets`); shared owns the resolver and the allowed preset-name set (`full` reserved = everything). Rot-proofing mirrors the Rule #28 `mutates` discipline: `assertPresetCoverage` throws at construction when `include ∪ exclude` ≠ the package tool set (a future tool CANNOT silently vanish from a preset), backed by per-package set-equality tests and a `TOOL_PAIRS` co-visibility guard (`iris_env_diff`/`iris_env_promote` ship together in every preset). Roster intents are recorded: `core` attacks the count cliff (every server ≤13 runtime tools — 12/12/9/9/7 package tools); `developer` attacks persona relevance (hides users/roles/resources/SSL/OAuth/LDAP/X509/audit + backup/mirror/ECP surfaces; 28/10/22/9/7). A new `scripts/measure-tools-payload.mjs` records the `tools/list` payload (count/bytes/~tokens) per server × preset — the first measured evidence behind the PRD's tool-selection-accuracy validation approach.

### Server Manager Workspace-Candidate Trust (Epic 32 burn-down, Story 32.3 — added 2026-07-26)

Deferred item `31-3-9` assigned this decision to Epic 32 planning, to be recorded here rather than in a story: with `IRIS_SM_WORKSPACE` unset, `discoverSettingsFiles` derives the workspace-scope candidate from the server process's CWD — which the MCP *client* (not the operator) chooses — so a merely-cloned third-party repository's `.vscode/settings.json` can contribute named connection profiles (an entry carrying an inline password becomes directly addressable via the `server` param). `IRIS_GOVERNANCE_FILE` (Story 32.0) raises the same "which directory do we trust?" question for the governance channel.

**J1 — Discovery-from-CWD is grandfathered for Epic 31's Server Manager channel; new config channels are explicit-path-only (decided 2026-07-26).** The CWD-derived workspace candidate STAYS for `IRIS_SERVER_MANAGER` in v1/v2: it is AC-31.0.1-mandated shipped behavior, no user complaint exists, and the mitigations are documented at the point of use (`IRIS_SM_WORKSPACE` to pin the workspace, `IRIS_SM_SERVERS` to allow-list importable names, env/`IRIS_PROFILES` definitions always win name collisions, the consulted candidate list logged at debug, and `source`/`sourceFile` provenance on the `iris_server_profiles` roster so an unexpected profile is attributable). Requiring an explicit opt-in for the workspace candidate is a BREAKING change, deferred to a future major and to be reconsidered only with real-user evidence. For NEW config channels the answer is different by design: `IRIS_GOVERNANCE_FILE` is an **explicit operator-supplied path, never discovered** — the "untrusted repo injects config" class does not arise, because nothing is read from a directory the client merely happens to launch in. *Rationale:* preserves back-compat (Rule #19) on the shipped channel while keeping the trust surface of every future channel minimal; a single recorded policy ("discovered for the one AC-mandated legacy case, explicit for everything new") prevents per-story re-litigation of the same question.

### Bulk-Upload Path-Mapping Self-Validation (Epic 35, Story 35.9 — added 2026-08-17)

Added via `sprint-change-proposal-2026-08-17-doc-load-package-prefix.md`, itself triggered by `docs/bugs-2026-08-17.md` (an externally-reported defect, not the Epic 35 sweep). `iris_doc_load` derives each uploaded document's IRIS name purely from where the caller's glob pattern happens to place its first wildcard character (`extractBaseDir`/`filePathToDocName`, `packages/iris-dev-mcp/src/tools/load.ts`) — a heuristic with no connection to the file's actual declared identity, so a common glob shape (a literal package directory immediately before a trailing `*.ext`) silently strips the package prefix and uploads succeed under the wrong name, with the failure only surfacing later as a confusing, seemingly-unrelated IRIS compile error.

**K1 — Content is the source of truth for bulk source-file uploads, not glob-pattern shape.** For file types that embed their own identity (`.cls`/`.mac`/`.int`/`.inc` — a `Class Pkg.Name Extends ...` or `ROUTINE Name` declaration), the path-derived document name is no longer trusted blindly: it is cross-validated against the content-declared name, and a divergence refuses the upload for that file with an explicit error naming both candidates, rather than uploading a mismatched or unqualified document silently. This establishes a general pattern for this codebase's bulk/path-mapping tools: **wherever an operation both (a) infers an identifier from a filesystem path AND (b) has an independent, authoritative identifier available in the artifact itself, the two must be cross-validated rather than trusting the path alone.** File types with no embedded identity (CSP/slash-style docs) keep pure path-derived naming — there is no ground truth to validate against. Paired with an additive, optional `baseDir` parameter so a programmatic caller (an agent, in particular) can state the package root deterministically instead of relying on glob-shape inference at all; omitted, behavior is byte-identical to today (Rule #19). This is a narrower, single-tool decision compared to H1–J1 above — recorded here because it changes `iris_doc_load`'s validation contract, not because it introduces new epic-scale mechanism.
