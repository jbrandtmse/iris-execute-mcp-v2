...
- **Story impact**: three new stories (34.0 probe, 34.1 ObjectScript handler, 34.2 TS tool + docs + smokes). No existing story re-opened.
- **Artifact conflicts**:
...
- **Technical impact**:
  - `src/ExecuteMCPv2/REST/Command.cls` is a **bootstrapped class** (`gen-bootstrap.mjs` roster line 26) → Story 34.1 must regenerate `gen:bootstrap` and record the BOOTSTRAP_VERSION from→to (Rule #24). Not a new class → no roster changes (Rule #39 not triggered).
...
Append after Epic 33: epic header + goal/scope/FR mapping + Stories 34.0/34.1/34.2 with ACs, as drafted and approved in-session. Key ACs: capture parity with `Execute()` incl. Rule #7 discipline (34.1.1); marker-object by-ref with `byRefValues` (34.1.2); 20-arg ladder with boundary tests 0/1/10/11/20 and count-21 rejection (34.1.3); Rule #19 back-compat pin (34.1.4); BOOTSTRAP_VERSION bump (34.1.5); TS schema/docs updates with unchanged counts/annotations (34.2.1–34.2.3); live-smoke epic gate = the three bug-report reproductions pass without wrapper classes, plus Output-param, 20-arg, and second-namespace runs (34.2.4, Rules #21/#22/#26/#34).

**Documentation scope (stakeholder-directed, 2026-08-14)**: AC 34.2.3 names every doc surface explicitly (audited by grep this session): `packages/iris-dev-mcp/README.md` in BOTH spots (tool summary table ~L213 and detail block ~L915); `docs/migration-v1-v2.md` mapping row ("Same functionality" → enhanced); `docs/tool_support.md` and root `README.md` verified to currently not mention the tool — the story records checked-no-edit-needed or adds coverage if warranted; CHANGELOG entry. Review re-asks the Rule #56 completeness question against these surfaces.

### Proposal 2 (APPROVED) — sprint-status.yaml: append Epic 34 block

```yaml
epic-34: backlog
34-0-classmethod-invocation-probe: backlog
34-1-handler-capture-byref-20args: backlog
34-2-ts-tool-docs-smokes: backlog
...
Disposition section recording acceptance, Epic 34 tracking, and that Story 34.2's live smoke re-runs the report's three reproductions as the epic-done gate.
...
- **Handoff**: SM applies the three approved artifact edits (done as part of this workflow). Development proceeds via `create-story` → `dev-story` → review for Stories 34.0–34.2 when the epic is scheduled.
