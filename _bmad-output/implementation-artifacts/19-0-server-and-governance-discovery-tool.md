...
- [x] [Review][Defer] Discovery ignores (and does not validate) `profile` when `allProfiles:true` [packages/shared/src/server-discovery.ts:computeServerDiscovery] — deferred, LOW. `{allProfiles:true, profile:"typo"}` silently returns all-profiles output without validating `profile`, inconsistent with the single-profile branch which throws ProfileResolutionError on a bad name. Not a correctness bug (allProfiles is a superset); fix choice is ambiguous (reject-as-mutually-exclusive vs document precedence). Recorded in deferred-work.md.
- [x] [Review][Defer] Invalid framework `server` arg hard-fails the discovery call before the short-circuit [packages/shared/src/server-base.ts:handleToolCall ~818 vs ~882] — deferred, LOW. The framework `server`→profile resolution runs for every tool before the discovery short-circuit, so passing an unknown `server` to discovery returns a ProfileResolutionError even though the tool's description states `server` is irrelevant to it and discovery never connects. The natural call omits `server`; impact is a minor contract wrinkle ("call discovery first to learn valid names" is defeated only if the client also guesses a bad `server`). Recorded in deferred-work.md.
