...
6. Restart the MCP server (Copilot: restart the "IRIS Dev Tools — <server>" server, or Reload Window) so it re-reads the file at startup. **The server under test must be launched with `IRIS_GOVERNANCE_FILE` in its env** — the VS Code extension's `governanceFile` passthrough covers servers VS Code spawns; a server registered in another client (e.g. Claude Code's `~/.claude.json`) needs the env var added to that registration (absolute path), then a server restart. (Amended 2026-07-27 during the smoke: the first step-7 run verified against the Claude Code registration, which lacked the env var — correct-by-design fail-fast-off behavior, but a setup gap the procedure must state.)
...
**AC 32.2.4 — RESULT: PASS (2026-07-27, Project Lead).** VS Code **1.128.0** · Server Manager **3.12.3** · suite commit `f0ddfab` (feature branch, plus the mid-smoke 32-2-U1 alignment fix in the working tree). Steps 4–8 all verified: missing-file banner + per-package universe render (step 4, incl. the engine's local-mode subprocess resolution in the real extension host); one `global (file)` toggle → Save → file on disk with exactly `{"global":{"iris_doc_put":false}}` (step 5 — single layer, answering the earlier double-layer question: the first run's `profiles.default` entry was a second toggle on the `default` tab, expected per-tab behavior); `iris_server_profiles` reports `policy.iris_doc_put === false` + `configSource.iris_doc_put === "file"` (step 7); `iris_doc_put` refused with `GOVERNANCE_DISABLED` (step 8). Step-7 initially FAILED against the Claude Code `~/.claude.json` registration, which lacked `IRIS_GOVERNANCE_FILE` — correct-by-design (explicit-path-only, J1), not a product defect; step 6's procedure text was amended to state the server-under-test must be launched with the env var. Cleanup (step 9) performed by the lead: `unset` via the CLI (editor-equivalent path), env var removed from `.claude.json`, `governanceFile` setting cleared, file + directory + backups removed. Two findings triaged from this run: 32-2-U1 (column zigzag — RESOLVED mid-smoke, one table per group, structural pin, VSIX reinstalled and the smoke completed on the fixed build) and the procedure preamble gap (amended in place).
