...
- **AC 33.3.4 (manual GUI smoke) — ✅ PASS, executed by the Project Lead 2026-07-28** (procedure below as executed). Versions: VS Code **1.128.0**, Cline extension **4.0.11**, Claude Code CLI **2.1.220**, `adapterDataVersion` **2026-07-28.1**, iris-mcp-launcher VSIX 0.1.0 (rebuilt with the post-merge UI amendment). Outcomes: step 1 VSIX build ✅ (step 2 install + `developmentRepoPath` ✅, with two real-world lessons: the settings file was double-rewritten by the running VS Code instance, and a same-version VSIX reinstall does NOT refresh the running bundle — uninstall→install→restart required); step 3 roster persistence ✅ (Kimi Code unchecked, survives reload); step 4 apply `iris-dev-mcp` (env-reference, user scope) to Claude Code + Cline via Preview→Confirm ✅ (restart hints shown; the Claude Code node dev entry updated in place, unmanaged keys preserved); step 5 `claude mcp list` shows iris-dev-mcp + Cline MCP panel connected ✅; step 6 disable for Cline only → absent in Cline after reload, still present in Claude Code ✅ (the F3 success metric); step 7 restore backup for Cline ✅ + Claude Code original node dev entry restored (verified post-smoke: `command:"node"`, dev-repo dist path, real env intact). Original procedure:
...
- AC 33.3.4 manual GUI smoke ✅ PASS 2026-07-28 (Completion Notes).
