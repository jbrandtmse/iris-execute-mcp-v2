...
- **104 tools** across 5 specialized MCP servers (up from 8 in a single server)
...
| **Architecture** | Single server, 8 tools | 5 servers, 104 tools total |
...
|------------|--------|------------|
| `@iris-mcp/dev` | Development, compilation, execution, globals, SQL, SQL analysis, package browsing, bulk export, macro-expanded routine lookup, lines-of-code metrics | 26 |
...
| `@iris-mcp/interop` | Interoperability productions, production items, system default settings, credentials, lookups, rules, transforms, message-trace diagrams | 21 |
