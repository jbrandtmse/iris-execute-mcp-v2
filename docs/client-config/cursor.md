...
> **Superseded by the manager (Epic 33):** the fastest, safest way to wire Cursor is
> `npx -y @iris-mcp/client-config iris-mcp-clients apply --client cursor --servers iris-dev-mcp`
...
> This page remains as the **manual fallback**. Cursor's adapter is
...
The file's layers sit strictly BELOW any `IRIS_GOVERNANCE` env value in the cascade, and the file is read once at server startup — restart Cursor (or the server) after editing. See [Governance file](../../README.md#governance-file-iris_governance_file) in the suite README for the full model.
