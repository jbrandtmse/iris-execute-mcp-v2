...
> **Escaping matters.** The *value* of `IRIS_PROFILES` / `IRIS_GOVERNANCE` is a JSON string that lives inside `claude_desktop_config.json`, so every inner `"` must be escaped as `\"`. The block below is already correctly escaped — copy it as-is and edit the values.
